...
export default function Events() {
  ...
  const handleSendBulkDM = async () => {
    if (!sharingEvent || selectedUserIds.length === 0) return;
    setSendingBulk(true);
    try {
      const textToSend = `Check out this event:\n"${sharingEvent.title}"\n\nSee it here: https://offcampus.in/events/${sharingEvent.event_id}`;
  ...
  const handleShareWhatsApp = async () => {
    if (!sharingEvent) return;
    const shareText = `Check out this event on Off-Campus: "${sharingEvent.title}"\n\nRead more at: https://offcampus.in/events/${sharingEvent.event_id}`;
    ...
    Linking.canOpenURL(url)
      .then((supported) => {
        if (supported) {
          ...
          return Share.share({ message: shareText });
        }
      })
